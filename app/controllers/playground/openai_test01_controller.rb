...

{
  ...
  "maker": 機械・工具のメーカー会社。別表記・類義語、略称、読み(カタカナ)、漢字違い(沢と澤、富と冨のような)があれば|区切りでできるだけ多く列挙(機械・工具名、型式に含まれる場合は除外),
  "model": 機械・工具の型式、機種。半角英数字大文字。それ以外の記号や漢字カナは除外,
...
・ ユーザは、工場で実際に加工作業を行う方を想定しています。
・ 各機種の違いや用途など、実用的な情報を記述してください。
・ 質問文の内容も考慮して、それに回答する内容にしてください。
・ 個別の機械・工具に関する情報は(ID:ID番号)を表記してください。
・ 丁寧で優秀な秘書のような語り口で回答してください。(黒髪ロングでメガネの美人秘書を想定)
・ 結果は<< >>で囲ってください。
'
...
# '.freeze
  ...
  def sort_for_chat(message, machines)
    ...
    begin
      ...
      # search
      @sort_machines = Machine.sales.where(id: @sort_array)
    rescue StandardError => e
      @error = e.full_message
    end
  end
  ...
  def machine_to_json_hash(machine)
    ...
    res.reject{ |key, value| value.blank? || value == '-' }
  end
